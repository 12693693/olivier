...
if __name__ == "__main__":
    ...
    connections_dict = {'random': 'random_algo.assign_house_random(houses, batteries)', 'greedy' : f'greedy_algo.assign_closest_battery({houses_list}, {batteries_list})', 'hillclimber' : 'random_hill_climber', 'simulated annealing': 'random_sa'}
    cables_dict = {'90 degrees': 'cable_90_degree.make_90_degrees_cables(houses, batteries)', 'random' : f'cable_random.random_try({houses_list}, {batteries_list})', 'search cables' : 'search_cables.run_search(houses, batteries)', 'further cables': 'further_cables.run_further(houses, batteries)', 'breadth first': 'cable_breadth.run(houses, batteries)', 'random try': 'cable_random.run(houses, batteries)'}
    ...
    # loop input later naar 100 zetten
        ...
        list_costs = []
        my_smartgrid.batteries_list = copy.deepcopy(batteries)
        my_smartgrid.house_list = copy.deepcopy(houses)
        houses_list = my_smartgrid.houses_list
        batteries_list = my_smartgrid.batteries_list
        ...
        for i in range(int(loop_input)):
            ...
            list_costs.append(list[0]['costs-shared'])
